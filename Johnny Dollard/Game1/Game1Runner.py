...
while True:
    ...
    drawText(font.render("+ SPEED: 1", 1, (0, 0, 255)),50,25)
    drawText(font.render("+ HP REGEN: 2", 1, (255, 0, 255)),175,25)
    font=pygame.font.Font(None, 48)
    drawText(font.render("COINS: "+str(coins), 1, (255,255,0)),683,20)
    drawText(font.render("HP: "+str(int(HP//1)), 1, (HPr, HPg, 0)),683,50)
    if enemy1OnScreen or enemy2OnScreen:
        drawText(font.render("TOTAL ENEMY HP: "+str(int(enemy1HP//1)+int(enemy2HP//1)), 1, (255,0,0)),683,80)

    #Make Coin
    if not(coinOnScreen) and randint(1,500)==1:
        coinOnScreen=True
        coinX=randint(10,1356)
        coinY=randint(10,758)
    ...
    if not(enemy1OnScreen) and randint(1,500)==1:
        ...
        enemy1HP=10
    #Make Enemy2
    if not(enemy2OnScreen) and randint(1,500)==1:
        enemy2OnScreen=True
    ...
    if enemy1OnScreen and enemy2OnScreen and enemy1X-enemy2X<48 and enemy1X-enemy2X>-32 and enemy2Y-enemy1Y>-48 and enemy2Y-enemy1Y<32:
        enemy2HP-=0.25
        ...
        # move guy in the correct direction
            ...
            if event.key==K_w:
                ...
                if coins<speed*speed*speed:
                    drawText(font.render("NOT ENOUGH COINS! Cost: "+str(speed*speed*speed), 1, (255, 127,0)),683,720)
                    pygame.display.update()
                        
                elif speed>=5.1:
                    drawText(font.render("ALREADY AT MAXIMUM VELOCITY!", 1, (255, 0, 0)),683,720)
                    pygame.display.update()
                        
                else:
                    drawText(font.render("Confirm Purchase \"SPEED +1\" for "+str(speed*speed*speed)+" Coins? ENTER/BACKSPACE", 1, (255, 255, 0)),683,720)
                    pygame.display.update()
                    for event in pygame.event.get():
                ...
                if coins<int(1000*regenSpeed*1000*regenSpeed*1000*regenSpeed):
                    drawText(font.render("NOT ENOUGH COINS! Cost: "+str(int(1000*regenSpeed*1000*regenSpeed*1000*regenSpeed)), 1, (255, 127,0)),683,720)
                    pygame.display.update()
                        
                elif regenSpeed>=0.0051:
                    drawText(font.render("ALREADY AT MAXIMUM HP REGEN SPEED!", 1, (255, 0, 0)),683,720)
                    pygame.display.update()
                        
                else:
                    drawText(font.render("Confirm Purchase \"HP REGEN SPEED +1\" for "+str(int(1000*regenSpeed*1000*regenSpeed*1000*regenSpeed))+" Coins? RETURN/BACKSPACE", 1, (255, 255, 0)),683,720)
                    pygame.display.update()
                    for event in pygame.event.get():
                        if event.type==KEYDOWN:
                            while not(event.type==KEYDOWN and (event.key==K_RETURN or event.key==K_BACKSPACE)):
